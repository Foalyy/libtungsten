...
namespace SCIF {
    ...
    void enableOSC0(unsigned long frequency, bool enabled) {
    ...
    void enablePLL(int mul, int div, GCLKSource referenceClock, unsigned long referenceFrequency, bool enabled) {
        if (enabled) {
    ...
    void enableDFLL(unsigned long frequency, GCLKSource referenceClock, unsigned long referenceFrequency, bool enabled) {
        if (enabled) {
    ...
    void enableRC80M(bool enabled) {
        // Configure RC80M
        (*(volatile uint32_t*)(SCIF_BASE + OFFSET_RC80MCR))
                = enabled << RCFASTCFG_EN;             // EN : enable the oscillator
    }
    ...
    void enableGenericClock(GCLKChannel channel, GCLKSource source, bool output, uint32_t divider, bool enabled) {
